...
namespace {
  ...
  private:
    ...
    SDValue visitWASM_MEMREF_ADD(SDNode *N);
...
SDValue DAGCombiner::visit(SDNode *N) {
  ...
  case ISD::WASM_MEMREF_ADD:  return visitWASM_MEMREF_ADD(N);
...
SDValue DAGCombiner::visitWASM_MEMREF_ADD(SDNode *N) {
  SDValue N0 = N->getOperand(0);
  SDValue N1 = N->getOperand(1);
  if (N1->getOpcode() == ISD::WASM_MEMREF_FIELD
      && N0.getOpcode() == ISD::WASM_MEMREF_NULL) {
    return N1->getOperand(1);
  }
  return SDValue();
}
